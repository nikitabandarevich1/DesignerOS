...
version: 1.0.0
description: |
  Post-ship documentation update for the Factorial webpage. Reads all project docs,
  cross-references the diff, updates README/ARCHITECTURE/CONTRIBUTING/CLAUDE.md,
  syncs DatoCMS component notes, logs new design system components to the team
  changelog, cleans up TODOs, and optionally bumps VERSION.
  Use when asked to "update the docs", "sync documentation", or "post-ship docs".
  Proactively suggest after a PR is merged or code is shipped. (designeros)
allowed-tools:
  - Bash
  - Read
  - Write
...
```bash
find . -maxdepth 3 -name "*.md" -not -path "./.git/*" -not -path "*/node_modules/*" -not -path "./.designeros/*" -not -path "./.context/*" | sort
```
...
**Factorial Webpage — extra checks:**
- **webpage/design-system/**: If new atoms, molecules, or organisms were added or changed, do any `README.md` or usage docs in that directory need updating?
- **DatoCMS component notes**: If a new CMS section type was added (new DatoCMS model), check whether `webpage/src/modules/<module>/README.md` or any design-system doc should note the schema.
- If new DatoCMS fields, blocks, or models were added: cross-reference with existing documentation about the CMS schema.
...
```bash
NEW_TITLE=$(~/.claude/skills/designeros/bin/gstack-pr-title-rewrite.sh "$V" "$CURRENT_TITLE")
```

...
version: 1.0.0
description: |
  Fast headless browser for QA testing and site dogfooding on the Factorial webpage.
  Navigate pages, interact with elements, verify state, diff before/after, take annotated
  screenshots, test responsive layouts, forms, uploads, dialogs, and capture bug evidence.
  Use when asked to open or test a site, verify a deployment, dogfood a user flow, or
...
### Webpage development
- User is building a new CMS section, DatoCMS component → invoke `/webpage-cms-component`
- User is building a new page, route, landing page → invoke `/webpage-page`
- User is working with design tokens, design system components, Storybook → invoke `/webpage-design-system`
- User needs a DatoCMS schema migration → invoke `/datocms-migration`
- User asks about general webpage architecture → invoke `/webpage`

### Animation & interaction
- User asks about whether/how to animate something, polish, interaction design → invoke `/design-engineer`
- User needs GSAP ScrollTrigger, pinning, scrub, scroll-linked → invoke `/gsap-scrolltrigger`
- User needs Framer Motion, whileInView, AnimatePresence, layout animation → invoke `/nextjs-framer-motion`
- User wants a full immersive scroll experience, parallax storytelling → invoke `/scroll-experience`
- User wants premium UI quality, hero architecture, entry sequences, magnetic interactions → invoke `/premium-frontend-ui`

### Meta-workflows (start here)
- User wants to start working on a ticket, project, or feature → invoke `/start`
- User wants to build a new CMS section end-to-end → invoke `/new-section`
- User wants to build a new page end-to-end → invoke `/new-page`
- User wants to animate a component or section → invoke `/animate-this`
- User wants a full design audit of the site or a section → invoke `/design-audit`
- User is ready to ship — wants final checks + PR + docs → invoke `/ship-it`

### Workflow
- User describes a new idea, brainstorms a feature, wants design feedback → invoke `/office-hours`
- User wants to review the plan's design quality, visual spec → invoke `/plan-design-review`
- User wants all reviews done automatically → invoke `/autoplan`
- User reports a bug, something is broken → invoke `/investigate`
...
- User asks about page speed, benchmarks → invoke `/benchmark`
- User asks to upgrade DesignerOS → invoke `/designeros-upgrade`
- User asks to save progress, checkpoint → invoke `/context-save`
- User asks to resume, restore context → invoke `/context-restore`
- User wants to draw/generate a design image → invoke `/design-shotgun`
- User wants to build a design in HTML/CSS → invoke `/design-html`
- User wants design advice, consultation → invoke `/design-consultation`
- User wants to challenge or reframe a brief → invoke `/scope-challenge`
- User wants release notes, changelog → invoke `/release-notes`
- User asks what DesignerOS has learned → invoke `/learn`
